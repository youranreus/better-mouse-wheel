...
export default class MouseWheel {
  ...
  private handleOptions() {
    ...
    this.updateExceptDOM();
  }

  private updateExceptDOM() {
    if (this.mouseWheelOpt.exceptDOM) {
      if (typeof this.mouseWheelOpt.exceptDOM === "string") {
        this.exceptDOM = Array.from(
  ...
  private handleHooks() {
    ...
    this.registerHooks(this.scroll.hooks, "refresh", this.updateExceptDOM);
  }
  ...
  private wheelHandler(e: CompatibleWheelEvent) {
    if (
      !this.scroll.enabled ||
      (e.target && this.checkException(e.target as HTMLElement))
    ) {
      return;
    }
