...
export default class MouseWheel {
  ...
  private handleOptions() {
    ...
    if (this.mouseWheelOpt.exceptDOM) {
      if (typeof this.mouseWheelOpt.exceptDOM === 'string') {
        this.exceptDOM = Array.from(document.querySelectorAll(this.mouseWheelOpt.exceptDOM)) as unknown as HTMLElement[];
      } else {
  ...
  private wheelHandler(e: CompatibleWheelEvent) {
    if (!this.scroll.enabled || (e.target && this.checkException(e.target as HTMLElement))) {
      return
    }
